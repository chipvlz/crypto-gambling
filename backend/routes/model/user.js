...
var getDateTimeStamp = function (date) {
...
var signup = function (data) {
    return new Promise((resolve, reject) => {
        ...
        var values = "('" + getDateTimeStamp(new Date()) + "', '"
        values += getDateTimeStamp(new Date()) + "', "
        ...
        var statement = db.statement("insert into", "users", "(CREATE_TIME, UPDATE_TIME, USERNAME, PASSWORD, EMAIL, AVATAR, REFERRAL_CODE, REFERRAL_CODE_P, WALLET, API_TOKEN)", "", "VALUES " + values)
...
var getUseridByParentReferralCode = function(referral_code_p) {
    var query = "SELECT ID FROM users WHERE REFERRAL_CODE_P='" + referral_code_p + "'";
    return new Promise((resolve , reject) => {
        db.con.query(query , function(err , result , fields) {
            if(err)
                reject(err);
...
var updateUserProfile = function(data = {} , filter = {}) {
    var sql = "update users set UPDATE_TIME = '" + getDateTimeStamp(new Date()) + "'  ";
    for(let[key , value] of Object.entries(data)) {
        if (key == 'LAST_VISIT') {
            value = getDateTimeStamp(new Date());
        }
        sql += " , " + key + " = '" + value + "' ";
    }
    sql += " where 1=1 " ;
